...
function fetchViaHost(payload){if(!hasHost)return Promise.resolve(null);const id='req_'+Date.now()+'_'+Math.random().toString(16).slice(2),timeout=payload?.streamTiming===true?60000:30000;return new Promise((resolve,reject)=>{pendingFetch[id]=resolve;post({type:'CPA_CHANNEL_FETCH',id,payload});setTimeout(()=>{if(pendingFetch[id]){delete pendingFetch[id];reject(new Error('扩展请求超时'))}},timeout)})}
