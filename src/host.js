...
function fetchViaHost(payload){if(!hasHost)return Promise.resolve(null);const id=payload?.requestId||'req_'+Date.now()+'_'+Math.random().toString(16).slice(2),timeout=payload?.streamTiming===true?60000:30000,nextPayload={...(payload||{}),requestId:id};return new Promise((resolve,reject)=>{const timer=setTimeout(()=>{if(pendingFetch[id]){delete pendingFetch[id];post({type:'CPA_CHANNEL_FETCH_CANCEL',id});reject(new Error('扩展请求超时'))}},timeout);pendingFetch[id]={resolve,reject,timer};post({type:'CPA_CHANNEL_FETCH',id,payload:nextPayload})})}
function cancelFetch(id){if(!id)return;const item=pendingFetch[id];if(item){clearTimeout(item.timer);delete pendingFetch[id];item.reject(new Error('请求已取消'))}if(hasHost)post({type:'CPA_CHANNEL_FETCH_CANCEL',id})}
...
window.addEventListener('message',e=>{const m=e.data;if(!m)return;if(m.type==='RELAY_STORE_DATA'&&m.data&&typeof m.data==='object'){applyStoreData(m.data);const sig=curStoreSig(),changed=!storeReceived||sig!==storeSig;storeSig=sig;storeReceived=true;if(!changed)return;listeners.store.forEach(fn=>fn(storeSnapshot(),sig));return}if(m.type==='RELAY_OPEN_MODE_DATA'){openModeValue=m.mode;openModeReceived=true;listeners.openMode.forEach(fn=>fn(openModeValue));return}if(m.type==='RELAY_HOST_CAPABILITIES'){hostCapabilities=normalizeCapabilities(m.capabilities||m.data);listeners.capabilities.forEach(fn=>fn(capabilities()));return}if(m.type==='RELAY_COPY_TEXT_RESULT'){const resolve=pendingCopy.shift();if(resolve)resolve({ok:!!m.ok,error:m.error||''});return}if(m.type==='RELAY_OPEN_SIDEPANEL_RESULT'){const resolve=pendingSidePanel.shift();if(resolve)resolve(m.response||{ok:true});return}if(m.type==='RELAY_OPEN_SITE_LOGIN_RESULT'){const resolve=pendingSiteLogin.shift();if(resolve)resolve(m.response||{ok:false,error:'宿主无响应'});return}if(m.type==='RELAY_READ_SITE_TOKENS_RESULT'){const resolve=pendingSiteTokens.shift();if(resolve)resolve(m.response||{ok:false,error:'宿主无响应'});return}if(m.type==='CPA_CHANNEL_FETCH_RESULT'&&m.id&&pendingFetch[m.id]){const item=pendingFetch[m.id];clearTimeout(item.timer);delete pendingFetch[m.id];item.resolve(m.response)}});
window.RelayHost={hasHost,inExtensionPage,getStore,setStore,requestStore,requestOpenMode,requestCapabilities,capabilities,hasCapability,setOpenMode,openSidePanel,copyText,fetch:fetchViaHost,cancelFetch,openSiteLogin,readSiteTokens,openExternal,onStoreData,onOpenModeData,onCapabilitiesData,storeSignature:()=>storeSig};
})();
