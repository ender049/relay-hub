...
部分站点会对 API 路径启用 Cloudflare Managed Challenge。此时即使已经读取到访问令牌、刷新令牌和 cookie，Rust 后端 `reqwest` 仍可能收到 `403 Just a moment...` HTML。

渠道编辑表单提供“浏览器请求模式”。开启后，Tauri 桌面客户端会从同步流程的第一条渠道请求开始，直接在已登录的 `WebView2` 登录窗口中执行 `window.fetch()`，复用该窗口的 cookie、WebView2 代理和已经通过的页面验证状态。

使用步骤：

1. 打开渠道编辑表单，填写站点、用户名和密码。
2. 点击“打开登录窗口”，在登录窗口完成 Turnstile 和登录。
3. 回到渠道表单点击“读取令牌”。
4. 勾选“浏览器请求模式”并保存渠道。

该模式在 Tauri 中只影响渠道请求，CPA 请求继续使用原有宿主请求路径。浏览器扩展也声明同名能力，并使用同域已登录标签页作为执行器。浏览器请求模式当前面向 JSON / 文本 API，请求或响应中的二进制 body 会返回明确错误。
