...
- 渠道操作区使用双列图标网格，包含 `手动刷新`、`自动刷新`、`密钥` 和 `更多`；监控入口为渠道 URL 后的无边框波形图标。
- 渠道监控按需读取：Sub2API 优先并发使用最近 90 分钟的 `/api/v1/channel-monitor-v2/snapshot` 和按平台/分组聚合的 `/matrix`；V2 不支持时仅请求一次 `/api/v1/channel-monitors` 展示 V1 分组级数据，不读取 V1 模型详情；New API 列表仅展示 `/api/perf-metrics/summary?hours=24` 中有性能采样的模型，状态取最近采样桶，展开模型时读取单模型 `/api/perf-metrics` 完整序列。
