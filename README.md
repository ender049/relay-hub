...
- 自动同步渠道余额、今日消费、累计消费、今日 Token、充值比例和可用分组；Sub2API 在今日数据后显示缓存读取 Token、总 Token 和占比，悬停读数可查看字段含义。
...
渠道余额、今日消费、累计消费、Token 和分组信息只来自聚合接口同步。NewAPI 不扫描调用日志，缓存信息保持隐藏；Sub2API 直接使用 dashboard stats 显示缓存读取 Token、今日总 Token 和两者占比。手动字段只用于补充充值入口和充值比例。
